...
| Authority | Reversible local: writes only the named throwaway local HTML artifact and, after the human validates the logic, the named real-code target committed on a throwaway branch based on the supplied main branch; rollback is version control. No remote mutation. Do not push, delete existing work, or change production rendering. |
