...
4. Apply the design-system color palette, spacing scale, and typography tokens from the references. Use only CSS custom properties or inline values that resolve without network access. Done when: design tokens are applied and resolve without network access.
