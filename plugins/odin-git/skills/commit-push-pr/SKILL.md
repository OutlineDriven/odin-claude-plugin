...
8. Preview and apply. Show the push target and commit list together with the PR target, base, head, title, and the first two sentences of the body, then wait for explicit confirmation. On approval, perform the push and then the PR create or edit; on decline or no answer, perform neither remote mutation. Write the body to a temp file and pass it with `--body-file`, never as an inline `--body` or through stdin, because wrappers can hand `gh` an empty body while it still exits 0 and prints a URL:
