...
3. For each author, take their latest submitted review (exclude PENDING and DISMISSED) and classify each of that review's comments by that review's own `state` field from `gh pr view --json reviews`: blocking (state is `CHANGES_REQUESTED`), suggestion (state is `COMMENT` and the comment is not a question), nit (state is `APPROVE` but the comment notes a minor issue), or question (the comment text ends with `?` or asks for clarification). Done when: every comment is classified into a tier by its owning review's state.
