...
3. For each author, take their latest submitted review (exclude PENDING and DISMISSED) and classify each of its comments by the review's state, using the PR-level review decisions from step 2: blocking (the review decision is `CHANGES_REQUESTED`), suggestion (the review decision is `COMMENT` and the comment is not a question), nit (the review decision is `APPROVE` but the comment notes a minor issue), or question (the comment text ends with `?` or asks for clarification). Use the review `state` field from `gh pr view --json reviews`, not thread-level states. Done when: every comment is classified into a tier.
