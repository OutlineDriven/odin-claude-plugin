...
| Authority | Human-gated: previews and obtains explicit human approval per action before any remote or irreversible action (push, PR creation or any other PR workflow mutation, release, deployment, credential use, branch or worktree deletion); approval for one does not authorize another; otherwise reversible local: executes the selected playbook's local actions in the working tree, including code edits, test runs, agent dispatch, and worktree creation, with version control as the rollback. |
