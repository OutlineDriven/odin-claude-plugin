...
6. Write the captured config to `.odin/config.local.yaml` as flat `pulse_*` keys. If the file exists, merge preserving non-pulse keys. If the directory or file does not exist, create it. Show the resulting pulse block and offer one round of edits. Done when: the config is written.
