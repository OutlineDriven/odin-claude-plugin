...
3. Collect commit history from the repository root, adjusting `--since` to the requested period: Done when: the stated action, evidence, and guard all hold.
   ```bash
   git --no-pager log --since="2 weeks ago" --pretty=format:"%h|%ad|%s" --date=short --stat
   ```
   Done when: commit history collected for the requested period from the repository root.
4. Collect active branches (work in progress), sorted by most recent commit: Done when: the stated action, evidence, and guard all hold.
   ```bash
   git --no-pager branch -r --sort=-committerdate | head -20
   ```
   Done when: active branches listed, sorted by most recent commit.
5. Collect recent merges to main (completed work): Done when: the stated action, evidence, and guard all hold.
   ```bash
   git --no-pager log --since="2 weeks ago" --merges --pretty=format:"%h|%ad|%s" --date=short main
   ```
   Done when: recent merges to main collected for the requested period.
...
9. Note in-progress work from active branches not yet merged to main. Done when: in-progress work from unmerged active branches noted.
