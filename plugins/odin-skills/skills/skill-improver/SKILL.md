...
| Authority | Reversible local: writes only files within the resolved skill directory and the `.code-improver/` run-artifact directory; rollback is restoring prior file content from the run artifact or version control. No remote mutation. No credential or paid mutation. |
