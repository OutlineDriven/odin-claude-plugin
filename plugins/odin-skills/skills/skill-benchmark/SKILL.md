...
description: 'Use when the user runs /skill-benchmark to score skills via LLM judges or compare candidate models on a shared task set. Not for release gating: use skill-benchmark-gate.'
...
| Side effect | Writes benchmark artifacts under .odin/benchmark-reports/ and incurs LLM inference spend. |
...
2. Create `.odin/benchmark-reports/` and `.odin/benchmark-reports/baselines/`. Done when: both directories exist.
...
9. Write and return the report. In skill-quality mode: write to `.odin/benchmark-reports/<date>-benchmark.md` and `.odin/benchmark-reports/<date>-benchmark.json`. In model-comparison mode: write the comparison table to the chosen output path (default `.odin/benchmark-reports/<date>-model-comparison.md`). Present the completed report or table to the human with its saved path. Done when: the files are written and their completed contents have been returned.
