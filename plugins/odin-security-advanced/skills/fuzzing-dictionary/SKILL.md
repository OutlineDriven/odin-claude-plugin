...
description: 'Use when a parser, protocol, or file format fuzzer stalls at fixed-token validation gates. Produces a fuzzer-parseable dictionary whose tokens correspond to target validation or parsing gates. Not for patching the SUT — use fuzzing-obstacles. Local writes only.'
---
...
4. Keep the dictionary focused: 50–200 of the most relevant entries. Deduplicate with `sort -u`. Drop full sentences and prose; keep atomic tokens. Done when: the dictionary has 50–200 deduplicated atomic entries.
...
6. Validate effectiveness: run a short campaign with and without the dictionary and compare coverage. Confirm the tokens reach the target's validation or parsing gates rather than being filtered before them. If a token is longer than the fuzzer's max input length, shorten it or raise the limit. Done when: coverage improves with the dictionary and tokens reach target gates.
