...
def _find_perf_binary():
    ...
    # The distro wrapper resolves the perf build matching the running kernel; use it when it works.
    try:
    ...
    # The wrapper fails after a kernel upgrade without matching linux-tools; a versioned binary still runs.
...
def run_perf_stat(kernel_package: str, op_path: str, warmup: int, iters: int, baseline_file: str | None = None) -> dict[str, float]:
    ...
    try:
        ...
        # perf stat writes its report to stderr, not stdout.
...
def print_analysis(stats: dict):
    ...
    # IPC bands are a heuristic for scalar-heavy or mixed code on Xeon cores. A tight
    # AVX512 FMA loop retires few, wide instructions per cycle by design, so low IPC
    # on such a kernel is not evidence of a memory bound; read it together with the
    # miss rates below before acting.
    if ipc < 1.0:
        ...
        recommendations.append(
            ">> IPC 1.0-2.0: moderate for scalar or mixed code.\n"
        ...
        recommendations.append(
            ">> IPC >= 2.0: the core is not stalling on this path.\n"
