---
name: observability
description: 'Use when adding telemetry, composing the smallest observability surface the system will keep, reviewing alerting rules, shipping a production feature, or diagnosing an opaque production issue. Not for diagnosing a live failure right now, profiling measured slowness, or launch-day runbooks.'
---
...
11. Keep only the surface the system will maintain: remove any instrumentation, label, span, or panel that duplicates another or that no owner will keep current. A surface no one maintains is not durable. Done when: no duplicate or unowned surface remains.
12. Verify the surface compiles, loads, and emits the intended signals against the running system or a representative test before any staged proof. Then verify the telemetry itself in a staging-like environment: force an error and find it in the logs by request ID with structured fields intact (no `[object Object]`); send test traffic and confirm the metric series appear with expected labels and sane values; follow one request end-to-end in the tracing UI with no broken spans; temporarily lower each new alert threshold, fire it once, and confirm it reaches the right channel with a working runbook link; then locate the induced failure using telemetry alone, without reading the source. Done when: the surface compiles, loads, and emits, and every staged check passes — error found by request ID, metrics appear with expected labels, one request traces end-to-end, alerts fire to the right channel, and the induced failure is locatable from telemetry alone.
...
Modified target source files carrying structured logs, metrics, tracing setup, alert definitions, and dashboard panels grouped by failure mode; the written on-call questions; the alert list with runbook links; and a verification report marking each check pass or fail. Terminal classification: done (all checks pass) or `blocked` (the named failing, unjustified, or unverifiable item).
