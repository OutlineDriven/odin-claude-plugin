...
- **Memory directory** (required): the directory to sanitize, supplied explicitly or resolved from the current project by `../memory-update/scripts/resolve-paths.sh memory_dir`. If `MEMORY_UPDATE_SKILL_SCRIPTS` is set, use `$MEMORY_UPDATE_SKILL_SCRIPTS/resolve-paths.sh` instead.
- Requires the sibling skill memory-update in the same plugin.
