...
1. Read the user's draft and any referenced repository context. Never invent an element: look things up in the request, conversation, or repository; an element that cannot be grounded is left out and flagged (see Failure and recovery). Done when: the stated action, evidence, and guard all hold.
2. Draft the condition as elements joined with AND, never "or": the loop takes the cheaper branch of an OR: Done when: the stated action, evidence, and guard all hold.
