...
1. **Validate inputs.** Stop if any role name is empty, any model name is empty, or the rule file path is not an absolute path. Verify each supplied model is available in the pstack model catalog; stop with `unavailable-model` if any model is not found. **Done when:** every role and model is non-empty, every model is available, and the path is absolute.
...
6. **Confirm done**: re-read the written file and verify every supplied role maps to its model and that every model is available. **Done when:** every supplied role resolves to an available model in the file.
