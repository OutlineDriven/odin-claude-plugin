...
**No backward compatibility [MANDATORY]:** Backward compatibility is not a goal. Build for the current requirements only. Migrate every caller inside the same change, and delete the old path instead of shimming, aliasing, dual-writing, or version-branching it. The posture gate above still holds.
