...
**LTS lines:** Node.js 24 (support to 2028-04-30), Java 25 (to 2030-09-30) and Java 21 (to 2028-09-30). Every other pin below names the current stable line; those projects run no LTS track.
...
**JavaScript (ES2026+):** ES2025 finished: iterator helpers, Set methods, `Promise.try`, `RegExp.escape`, import attributes (JSON modules), `Float16Array`. ES2026 adds Temporal and explicit resource management. ESM default. Runtime: Node.js 24+ (native TS type-stripping), Deno 2+, Bun 1+. Test: node:test. Lint: ESLint 10+ (flat config) | biome. Pkg: pnpm | npm.
...
**Java 25+:** Records, sealed, pattern matching, virtual threads, scoped values, AOT cache, compact headers. Immutability-first; Streams; Optional returns. Test: JUnit 6+Mockito+AssertJ. Lint: Error Prone+NullAway/Spotless. Security: OWASP+Snyk.
→ **Spring Boot 4.1+:** Virtual threads. RestClient, JdbcClient, RFC 9457. JPA+Specifications. Lambda DSL security, Argon2, OAuth2/JWT. Testcontainers.
**Kotlin 2.4+:** K2 (K1 removed in 2.4); JVM LTS target (21/25). val, persistent collections; sealed/enum+when (guard conditions); data classes; context parameters (stable; context arguments/callable refs still experimental); `@JvmInline`; inline/reified; non-local break/continue; multi-dollar string interpolation. Errors: Result, Either/Raise (Arrow); never !!/unscoped lateinit. Concurrency: structured coroutines, SupervisorJob, Flow, StateFlow/SharedFlow. Build: Gradle 9+ KTS+Version Catalogs; KSP2>KAPT (KAPT deprecated). KMP+Compose Multiplatform (iOS stable). Test: JUnit 6+Kotest+MockK+Testcontainers. Lint: detekt+ktlint. Libs: kotlinx.{coroutines,serialization,datetime,collections-immutable}, Arrow, Koin/Hilt.
**Go 1.27+:** Context-first; goroutines/channels clear ownership; worker pools backpressure; errors %w typed/sentinel; interfaces=behavior. Concurrency: sync, atomic, errgroup. Test: testify+race detector. Lint: golangci-lint/gofmt+goimports. Tooling: go vet; go mod tidy.
