...
**Workflow:** fd (discover) → `global --local -u` (indexed navigation) → ast-grep/rg (search) → Edit (transform) → git (commit) → git-branchless (manage)
...
### 7) Code Indexing
* **`gtags`**: Cross-reference DB. **LOCAL mode only.** Index: `gtags --local`. Query: `global --local <sym>` (defs), `global --local -r <sym>` (refs), `global --local -x <sym>` (xref)
* **`ctags`**: Tag file generator. 200+ languages. Index: `ctags -R .` | `ctags --exclude=node_modules -R .` | `ctags --languages=TypeScript -R src/`
