...
description: 'Use when competing revisions need frozen-evaluation, holdout-tested, resumable selection. Produces a Versioned experiment with a fresh-holdout champion and resumable best checkpoint. Stops at the declared success, non-success, or bound.'
---
...
2. If authority.approval is not null, collect start approval with the harness question tool once using the A1 sealed_fields list; end the run on scope drift. **Done when:** start approval is collected or authority.approval is null.
3. Execute the Versioned experiment with a fresh-holdout champion and resumable best checkpoint inside the bound. Confirm outcome.success holds or a named non_success/bound terminal applies. **Done when:** the experiment completes with a terminal class.
4. Stop at outcome.success, any outcome.non_success, or outcome.bound. **Done when:** a terminal class is recorded.
5. Persist per profiles.persistence.P1 (durable_location .outline/loops/<slug>/<run_id>/ when durable; emit receipt.json before return). Write an immutable K11 receipt with every K11 field. **Done when:** receipt.json is emitted before return.
