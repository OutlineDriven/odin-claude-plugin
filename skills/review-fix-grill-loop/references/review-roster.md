...
**Scope contract for this skill:** reviewers operate over the **resolved change-set only** — the working-tree + branch-vs-base diff plus untracked-not-ignored files (`changedFiles[]`), never the whole repository. Each reviewer receives that file set and the diff as its universe. Select at most **10 reviewers total** (4 core + up to 6 conditional).
...
File filter: all changed source files in scope; skip generated/vendor/minified assets unless directly changed or imported by entry-points.
...
Agent type: `reviewer`.
Activation: CORE, always.
File filter: auth/authz, validation, API routes, handlers, config, secrets, storage, serialization, templates, dependency loading, entry-points; include any changed file named by priority entry-point signals.
...
Agent type: `reviewer`.
Activation: CORE, always.
File filter: changed hot paths, loops, parsers, IO, database access, rendering paths, background jobs, entry-points, files named by pain/hotspots.
...
Agent type: `reviewer`.
Activation: CORE, always.
File filter: changed tests plus changed source files named by test-gap, bugspot, or changed-file signals.
...
- Regression tests needed for critical/high findings fixed by this run.
...
Activation: CONDITIONAL when changed file count > 50, cross-file slop clusters exist in the diff, or graph impact is broad.
File filter: changed module boundaries, package roots, core abstractions, dependency edges, files with high fan-in/fan-out.
...
File filter: changed schemas, migrations, queries, ORM models, repositories, transactions, seeders, database config.
...
File filter: changed routes, controllers, handlers, OpenAPI/spec files, clients, serializers, request/response schemas, middleware.
...
File filter: changed UI components, state stores, hooks/composables, client routes, form logic, browser API usage, styles when behavior-affecting.
...
File filter: changed services, jobs, queues, domain logic, server handlers, schedulers, adapters, integrations.
...
Agent type: `reviewer`.
Activation: CONDITIONAL when `CICD` or changed deployment/runtime entry-points exist.
File filter: changed CI workflows, Dockerfiles, compose/k8s/deploy manifests, package scripts, release scripts, infra config, environment templates.
