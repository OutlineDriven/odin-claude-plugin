...
description: 'Document a training session or workshop (compte rendu): Markdown draft iterated with the user, then a .docx generated once after explicit content confirmation. Not for remote, credential, publish, deploy, or irreversible changes.'
---
...
10. Deliver both files. Print the absolute paths to both the .md and .docx. List any images needing manual insertion with instructions. Remind the user that future edits go through the .md first. **Done when:** both file paths are printed and image instructions are provided.
