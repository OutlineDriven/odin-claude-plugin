...
description: 'Use when asked to identify and safely bypass checksums, nondeterminism, or validation barriers that block fuzzing coverage, then patch the System Under Test behind an explicit fuzzing build flag so coverage improves with false-positive risk assessed. Don''t use for remote, credential, publish, deploy, or irreversible changes.'
---
...
2. **Add conditional compilation gated on the fuzzing build flag.** Wrap the obstacle so it is enforced in production and bypassed only under the fuzzing build mode. Use the canonical flag for the target language:
   ...
   Deterministic PRNG seeding (C/C++) — replace a system-seeded `srand(time(NULL))` with a fixed seed under the fuzzing flag so the same input always produces the same behavior.

3. **Patch incrementally.** Bypass one obstacle at a time. Keep cheap validation (magic bytes, size checks) that guides the fuzzer at low cost; skip only the specific check that blocks coverage.

4. **Provide safe defaults when downstream code assumes validated state.** If code after the skipped check depends on a validated property (e.g., a divisor is nonzero), supply a safe fallback value under the fuzzing branch instead of skipping wholesale:
   ```c++
   #ifndef FUZZING_BUILD_MODE_UNSAFE_FOR_PRODUCTION
...
5. **Verify coverage improvement.** Rebuild with fuzzing instrumentation, run the fuzzer for a short time, and compare line/basic-block/function coverage and corpus diversity against the unpatched baseline. Confirm new code paths are reachable.

6. **Assess false-positive risk.** For each patch, determine whether skipping the check introduces program states impossible in production: does downstream code assume validated properties; could skipping cause crashes that cannot occur in production; is there implicit state dependency. Classify risk (LOW / MEDIUM / MITIGATED) and record it. If risk is high, narrow the patch or restore validation with safe defaults rather than skipping.

7. **Document each patch** so the fuzzing-vs-production divergence is visible to future maintainers.
...
## Output
A set of conditional-compilation patches in the System Under Test, each gated on the fuzzing build flag and documented; a coverage delta showing improved reachability over the unpatched baseline; and a per-patch false-positive risk classification (LOW / MEDIUM / MITIGATED) with rationale. Unresolved obstacles are reported as such with evidence rather than marked done.
