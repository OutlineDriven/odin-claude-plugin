...
description: 'Use when a significant feature begins or specs are checked in: author or update behavioral spec files and keep them current with what ships. Don''t use for remote, credential, publish, deploy, or irreversible changes.'
---
...
1. **Bound the scope.** From the feature brief and scope anchor, enumerate every file that will be created or modified. If the scope is ambiguous, stop and state the ambiguity rather than inferring a larger surface.
2. **Audit existing specs.** For each spec file already present in the affected scope, verify it is behavioral: each statement describes observable behavior or acceptance criteria, not implementation design. Discard or narrow any spec line that prescribes a solution path.
3. **Identify spec gaps.** Map every feature-brief item to at least one behavioral statement. Mark any item that cannot be expressed as observable behavior as out of scope.
4. **Write or update spec files.** For each gap, add a new spec section with:
   - a unique label,
   - the behavioral statement,
   - any preconditions and expected outcomes,
   - the pass/fail criterion that a test could verify.
   Place each spec in the file that most directly covers the affected surface.
5. **Verify spec-implementation alignment.** Confirm that no spec file contradicts an existing implementation. If a conflict exists, surface it before writing.
6. **Write or confirm the implementation.** Implement the feature to satisfy the spec. Do not add behavior not described by a spec.
7. **Commit with a spec-referencing message.** VCS-commit the spec and implementation files with a message that names the spec label(s) satisfied.

## Failure and recovery
- **Spec drift**: implementation adds behavior not covered by any spec. Non-converged result. Remove the undocumented behavior or add a covering spec before proceeding.
