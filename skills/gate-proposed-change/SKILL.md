...
description: 'Use when asked to evaluate a proposed commit, merge, or auto-merge against a version-1 gate config. Returns an allow or deny verdict naming the matched paths; the gate performs no merge. Don''t use for tasks that require source or remote-system changes.'
---
...
1. Validate the action is one of `commit`, `merge`, `auto-merge`. Stop with an invalid-action error on any other value.
...
4. If no condition denied, allow with trigger `ok`.
