...
2. **Infer project name and slug.** Probe in order and use the first match: `package.json` `name` field, `pyproject.toml` `project.name`, `Cargo.toml` `package.name`, `go.mod` module path (last segment after `/`), then the repository directory name as fallback. Derive the slug by lowercasing and replacing spaces and underscores with hyphens. Keep the human-readable name distinct from the slug. **Done when:** a human-readable name and a derived slug are recorded.
