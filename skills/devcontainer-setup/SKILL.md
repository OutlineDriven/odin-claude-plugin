...
description: 'Use when the user asks to add a new devcontainer or isolated Claude Code development environment to a repository that does not already have one, for Python, Node/TypeScript, Rust, Go, or a supported combination. Generates the five .devcontainer files with project substitutions and merged language-specific configuration. Don''t use for remote, credential, publish, deploy, or irreversible changes.'
---
...
1. **Refuse modification scope.** If `.devcontainer/` already exists, stop: this skill only adds a new devcontainer. Direct the user to edit the existing configuration for modifications. Do not trigger for general Docker, production container, or container lifecycle questions.

2. **Infer project name and slug.** Probe in order and use the first match: `package.json` `name` field, `pyproject.toml` `project.name`, `Cargo.toml` `package.name`, `go.mod` module path (last segment after `/`), then the repository directory name as fallback. Derive the slug by lowercasing and replacing spaces and underscores with hyphens. Keep the human-readable name distinct from the slug.

3. **Detect the language stack.** Detect each language from its manifest: Python from `pyproject.toml`, `requirements.txt`, `setup.py`, or `*.py`; Node/TypeScript from `package.json` or `tsconfig.json`; Rust from `Cargo.toml`; Go from `go.mod` or `go.sum`. Record every detected language; multi-language projects configure all of them.

4. **Generate the five files from the base templates.** Substitute `{{PROJECT_NAME}}` with the human-readable name and `{{PROJECT_SLUG}}` with the slug in Dockerfile, devcontainer.json, post_install.py, .zshrc, and install.sh. The base template carries Claude Code with marketplace plugins, sandboxing via bubblewrap and socat, Python via uv, Node via fnm, ast-grep, network isolation tools, security mounts (`.devcontainer/` mounted read-only to prevent container escape), token forwarding of `CLAUDE_CODE_OAUTH_TOKEN` and `ANTHROPIC_API_KEY` via `remoteEnv`, and modern CLI tools.

5. **Re-ground pinned toolchain versions and security-review broad privileges.** Do not copy pinned toolchain versions or broad container privileges (NET_ADMIN capability, iptables/ipset, bubblewrap sandboxing) verbatim from the source template. Confirm each pinned version against the current upstream release the project targets, and confirm each broad privilege is justified for this project's isolation needs; narrow or drop privileges that are not justified. Record the chosen versions and privilege set in the generated files.
...
7. **Compose postCreateCommand.** Chain every detected language's setup segment with `&&`, preserving the priority order, so all required setup runs on container creation. For a Python-plus-Node project this yields, for example, `uv run /opt/post_install.py && uv sync && pnpm install --frozen-lockfile`.

8. **Validate before presenting.** Confirm every `{{PROJECT_NAME}}` and `{{PROJECT_SLUG}}` placeholder is replaced; `devcontainer.json` is valid JSON with no trailing commas and correct nesting; language-specific extensions are present for every detected language; no extension or setting is duplicated; `postCreateCommand` includes all required setup commands chained with `&&`; the security mounts and token-forwarding `remoteEnv` are explicit.
...
## Output
Five files written under the repository's `.devcontainer/` directory: `Dockerfile`, `devcontainer.json`, `post_install.py`, `.zshrc`, and `install.sh`, with all placeholders resolved, all detected languages represented without duplicate configuration, a composed `postCreateCommand`, explicit security mounts and token forwarding, and re-grounded toolchain versions and reviewed privileges. A short report listing the detected languages, the chosen toolchain versions and privilege set, and the VS Code and CLI startup instructions. No claim that the container was built or launched unless a build or launch was performed.
