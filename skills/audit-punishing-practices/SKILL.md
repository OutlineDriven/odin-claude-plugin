...
description: 'Use when a workflow, plan, or diff must be checked for practices that punish the project later. Names each detected harmful practice with evidence and cheaper alternative, or reports the artifact clean. Not for source or remote-system changes.'
---
...
4. If no practice class matches, report the artifact clean with the scan coverage stated. Done when: the clean verdict names the five classes scanned and the artifact scope covered.
5. Emit the report as chat output only. Do not edit, stage, commit, or open issues. Done when: the report is emitted as chat output and no mutation occurred.
...
- Non-convergence does not apply: this is a single-pass read-only audit, not an iterative fix loop. It does not retry, and because it mutates nothing, it needs no rollback.
