...
description: 'Use when the user asks to cherry-pick, backport, or apply a hotfix to a release branch. Don''t use for merging feature branches or cutting new release branches.'
...
1. Resolve the supplied reference to the exact commit SHA(s) to apply. If a PR number or issue reference yields more than one commit, confirm the subset with the user before proceeding.
2. For each requested release channel, confirm its base release branch exists and is current, then fetch and check it out.
3. Preview the target and consequence to the user: the channel(s), the commit SHA(s), the branch names that will be created, the push, the PR titles, and the on-call reviewer assignment. Proceed only after explicit human confirmation.
4. For each release channel, create a cherrypick branch off that channel's base branch and cherry-pick each resolved commit in source order.
5. If a cherry-pick conflicts, stop that channel immediately; do not resolve, force, or skip it. Leave the working tree in the conflicted state and report it for user resolution.
6. Push each non-conflicted cherrypick branch to the remote.
7. Open one PR per non-conflicted channel against that channel's base branch, titled to identify the cherry-pick and the source reference, and assign the on-call reviewer.
8. Report the set of opened PRs and the set of channels stopped on conflict.
...
- Partial result: channels already pushed and PR-opened remain; stopped channels are reported, not silently dropped. The done predicate holds only for channels that reached an opened, assigned PR.

## Output
A report listing, per requested release channel: the cherrypick branch, the PR URL and title with the on-call reviewer assigned, or the conflict that stopped that channel. Terminal classification is `done` when every requested channel has an opened, correctly titled, reviewer-assigned PR, otherwise `blocked` with the stopped channels named.
