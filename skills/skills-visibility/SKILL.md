...
description: 'Use when asked to publish a discoverable integrity-protected agent-skill catalog served from a publisher-controlled domain. Produces .well-known/agent-skills/index.json with SHA-256 digests, flat archives for multi-file skills, and verified install commands. Don''t use for remote, credential, publish, deploy, or irreversible changes.'
---
...
## Output
A complete publishable tree in the output directory:
- `.well-known/agent-skills/index.json`: valid `application/json` conforming to the discovery schema.
- `agent-skills/<name>/SKILL.md` for each single-file skill.
- `agent-skills/<name>.tar.gz` for each multi-file skill (flat archive, deterministic build).
- Optionally `agent-skills/<bundle>-bundle.tar.gz` for bundles.
- A report listing each skill, its shape, its digest, and the install commands that were verified.
