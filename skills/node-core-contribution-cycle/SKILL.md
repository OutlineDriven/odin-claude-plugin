...
description: 'Use when a human explicitly invokes this workflow on a nodejs/node checkout to build core, change src/, lib/, or doc/api/, add a CLI flag, draft commits or a pull request, or review a core pull request. Don''t use for automated or unattended runs; publishing or using credentials requires explicit human direction after a preview.'
...
1. Confirm the checkout is nodejs/node, record the base revision and current changed-file set, and classify the task as source/library, `doc/api`, CLI, build/configuration, commit/PR drafting, or review. Bound all edits, builds, commits, and publication to the named checkout and stated contribution.
2. Read the checkout's contribution guidance and the nearest implementation, tests, documentation, configure logic, and build rules for the affected subsystem. Reuse its established command and file patterns rather than introducing a second convention.
3. Configure with the checkout's supported configuration entry point and only the options required by the target platform or change. When build-system or configure behavior changes, update the owning configuration/build declaration and verify both the generated build behavior and the user-visible option contract.
...
5. After every edit under `src/` or `lib/`, rebuild Node from the configured checkout before continuing. Stop on the first compile or link failure and report the failing command and diagnostic; do not treat a stale binary as evidence.
6. Keep `doc/api` synchronized with the observable behavior and local documentation structure. Examples, option names, defaults, stability claims, and cross-references must match the implemented interface; do not claim behavior that was not exercised or established by the checkout.
7. Run the focused tests or scenarios covering the changed behavior. Then run `make lint`. Compute the merge base against the supplied base revision, run the checkout's clang-format workflow over that diff, and require the resulting formatting diff to be empty. Record exact commands and outcomes.
8. For review work, compare the stated intent with implementation, tests, documentation, CLI/configuration effects, build portability, and protected-built-in usage. Separate blocking correctness findings from non-blocking suggestions, cite the affected location and observable consequence, and do not approve when required evidence is absent.
9. Create each commit with DCO sign-off. Use an imperative `subsystem: subject` no longer than 50 characters and a why-focused body wrapped to 72 columns. Run the checkout's `core-validate-commit` check for every commit and amend locally until it passes; do not bypass or suppress a failed check.
10. Before any push or pull-request creation, show the remote repository, branch, commit range, target branch, title/body, and publication consequence. Publish only the exact preview explicitly directed by the human. The pull-request description states the problem, rationale, observable change, tests and commands run, and any genuine limitation; it contains no invented evidence.
11. Finish only when the rebuild cadence and all lint, formatting, test, DCO, commit-validation, documentation, and review obligations applicable to the changed surface are evidenced and the final changed-file and commit sets remain within scope.
...
## Output
Return the bounded changed-file and commit sets; the base and merge-base revisions; rebuild commands run after each `src/` or `lib/` edit; focused test, `make lint`, clang-format-diff, DCO, and `core-validate-commit` results; documentation and CLI/configuration surfaces updated; review findings when applicable; and publication coordinates plus confirmed remote state when publication was explicitly directed. End with exactly one terminal classification: `complete`, `blocked`, `failed-checks`, or `publish-failed`.
