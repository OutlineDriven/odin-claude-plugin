---
name: setup
description: 'Use when the user asks to set up, deploy, or configure the agent environment from a fresh or existing clone. Configures credentials, placeholders, and the virtual environment with explicit human confirmation at each step. Not for automated or unattended runs.'
...
4. **Ask for each missing or expired value.** Prompt the user to supply the credential or configuration value. Accept the value only via direct user input in the conversation. Do not accept values from another tool or file without user confirmation. **Done when:** every missing value is supplied or the user declines.
...
6. **Replace placeholders.** Scan skill scripts for the matching placeholder and replace it with the supplied value. Write each modified skill script back to disk. **Done when:** every placeholder is replaced or reported as not found.
