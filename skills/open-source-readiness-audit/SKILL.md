...
description: 'Use when the user asks whether a repository is ready for public release or wants a gap assessment. Returns a prioritized readiness report covering secrets, documentation, licensing, CI, packaging, and release gaps. Don''t use for tasks that require source or remote-system changes.'
---
...
8. **Compile the report.** Organize findings by severity:
   ...
   Mark each gap as blocker, recommended, or deliberate omission. State which omissions are intentional choices for this project's scope.
...
## Output
A prioritized readiness report containing:
- Secret-history risk assessment with specific findings.
- Outsider build and use readiness: can someone with no prior context clone, build, and use the project?
- License consistency across LICENSE file, package metadata, and README.
- CI and supply-chain posture: test coverage, workflow hardening, dependency management.
- Packaging and release gaps: versioning, CI-driven releases, trusted publishing.
- Which omissions are deliberate choices versus genuine gaps.
