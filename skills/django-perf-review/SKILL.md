...
4. Detect queryset misuse: querysets evaluated more than once on the same path, `len(qs)`/`list(qs)` before iteration, slicing after evaluation, `.count()` where `.exists()` would suffice, missing `.iterator()` on large result sets, `.only()`/`.defer()` that still triggers deferred-field loads. **Done when:** queryset misuse candidates are identified.
5. Detect unbounded reads: list/index views without pagination, admin actions without queryset scoping, filter/order fields without a matching database index. **Done when:** unbounded reads are identified.
