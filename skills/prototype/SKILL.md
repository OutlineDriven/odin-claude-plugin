---
name: prototype
description: 'Use when asked to prototype: when one design question needs a cheap logic or UI experiment, build a throwaway artifact, record the one-line verdict, and fold the decision into real work without polluting the main branch. Don''t use for remote, credential, publish, deploy, or irreversible changes.'
---
...
Route before writing. Wrong routing wastes the whole prototype because LOGIC and UI variants produce structurally different artifacts.
...
2. If ambiguous and the user is not reachable, default: backend module → LOGIC; page or component → UI. State the assumption at the top of the artifact.
...
1. **State the question.** Write one paragraph at the top of the demo stating the exact question and the state model being tested. This is the checkpoint: if the prototype answers the wrong question it is pure waste.
...
3. **Build the shareable HTML file.** One file, plain HTML/CSS/JS, everything inline, opens by double-click. No framework, no bundler, no server.
...
4. **Hand it over.** Surface the file. Non-developers (designer, PM, domain expert) should be able to drive it without assistance.
5. **Capture.** When the prototype answers the question: fold the validated reducer/machine/function set into the real module; commit the HTML file and the verdict to a throwaway branch off main. The main branch keeps only the folded decision.
...
1. **State the plan.** Write one line stating the variants, the `?variant=` switcher key, and the route or page. Place it in the prototype's location or as a top-of-file comment.
2. **Default to 3 variants.** Cap at 5. More than 5 stops being radically different and starts being noise.
3. **Draft each variant.** Hold each to the page's purpose and data, the project's component library or styling system, and a clear exported component name (`VariantA`, `VariantB`, `VariantC`). Variants must be structurally different — different layout, different information hierarchy, different primary affordance. Not just different colours or copy. If two drafts come out too similar, redo one with explicit "do not use a card grid" guidance.
4. **Choose sub-shape:**
...
5. **Wire the switcher.** On the route, render all variants conditionally and add the floating switcher:
   ...
   ```
6. **Build the floating switcher.** Small fixed-position bar at bottom-centre with left arrow, variant label, and right arrow. Clicking an arrow updates the URL search param so the variant is shareable and reload-stable. Keyboard `←` and `→` also cycle; do not intercept when an `<input>`, `<textarea>`, or `[contenteditable]` is focused. Gate on `process.env.NODE_ENV !== 'production'` so the bar never ships to users.
7. **Hand it over.** Surface the URL and the `?variant=` keys.
8. **Capture.** When a variant wins: fold the winner into the real code. On sub-shape A, drop the losing variants and the switcher from main. On sub-shape B, promote the winning variant to a real route. Commit all variants and the verdict to a throwaway branch off main.

---

### Shared rules (apply to both LOGIC and UI)

1. **Mark it throwaway from day one.** Name it and locate it so a casual reader immediately knows it is not production code.
2. **Trivial to run.** UI prototype starts from one project command (`pnpm <name>`, `python <path>`, `bun <path>`). Logic prototype is one HTML file the user double-clicks. No thinking required.
3. **No persistence by default.** State lives in memory. If the question explicitly involves a database, use a scratch DB or local file with a clear "PROTOTYPE, wipe me" name.
4. **Skip the polish.** No tests, no error handling beyond what makes the prototype runnable.
5. **Surface the state.** After every action (logic) or on every variant switch (UI), display the full relevant state so the user can see what changed.
6. **Capture the evidence off main.** Commit the prototype to a throwaway branch. The main branch keeps only the validated decision.
...
## Output
- **Decision**: one line answering the question.
- **Evidence**: the prototype HTML file (LOGIC) or variant components and switcher (UI), committed to a throwaway branch off main.
- **Verdict record**: the question, the answer, and the chosen path recorded in the issue or commit message on the throwaway branch.
