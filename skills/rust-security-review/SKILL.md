...
description: 'Use when the user requests a security or correctness audit of a Rust crate, service, library, or Rust subtree, especially unsafe blocks, FFI, concurrency, async runtimes, or untrusted inputs: run read-only analysis and write scoped findings and SARIF artifacts without changing audited production code. Don''t use for remote, credential, publish, deploy, or irreversible changes.'
---
...
11. Assemble a Markdown report that includes scope, coverage, severity classification, surviving findings, and any incomplete workers. Write it to the review directory as the primary artifact.
12. Return the review directory path as the output.
