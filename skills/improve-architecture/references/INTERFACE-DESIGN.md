...
Uses the vocabulary in `codebase-design` — **module**, **interface**, **seam**, **adapter**, **leverage**.
...
Include both `codebase-design` vocabulary and `CONTEXT.md` vocabulary in the brief so each sub-agent names things consistently with the architecture language and the project's domain language.

Each sub-agent outputs:

1. Interface (types, methods, params — plus invariants, ordering, error modes). Express in the codebase's language; if the project is polyglot, give the surface in each owned language family (e.g., a Kotlin sealed interface plus a Go interface signature for a service crossing both).
