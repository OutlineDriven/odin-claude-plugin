...
description: 'Use when asked to render a GitHub PR as standalone review HTML. Fetches PR data via gh API, renders diffs with move detection, and serves locally. Don''t use for remote, credential, publish, deploy, or irreversible changes.'
---
...
1. **Parse the PR identifier.** Extract `{owner}`, `{repo}`, `{number}` from the provided URL or `owner/repo#number` string. Stop if extraction fails.
...
2. **Fetch PR data in parallel.** Run these `gh api` calls concurrently:
   ...
   ```
   Stop if any call fails or returns no data.
...
3. **Analyze the PR and write body HTML.** Read the diffs, understand the PR, and write the `<body>` content directly as HTML. Use any structure that best fits the PR:
   ...
   Include `<div data-diff="<target>">` placeholders where diffs should render. Add collapsible boilerplate sections, inline code references, callout boxes for warnings, and any other content that makes the review clear.
...
5. **Assemble the final HTML.** Write the body fragment from step 3 to `/tmp/pr-review-<number>-body.html`. Save patches to `/tmp/pr-patches-<number>.json` using `jq`:
   ...
   ```
   Escaping `<`, `>`, and `&` prevents patch text from terminating the JSON script element.

6. **Serve locally on a fixed port.** Start `python3 -m http.server 8432 --bind 127.0.0.1` with the harness background-process manager and `/tmp` as the working directory. If port 8432 is taken, try 8433, then 8434. Stop after 8434 fails and report the attempted ports. Report the ready URL to the user.
...
## Output
- File: `/tmp/pr-review-<number>.html` — self-contained HTML with embedded CSS, embedded JS, and safe JSON patch data.
- URL: `http://127.0.0.1:8432/pr-review-<number>.html` (or next available port).
- Done: the interactive risk-first review canvas renders in the browser.
