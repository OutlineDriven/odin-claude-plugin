...
description: 'Use when a user asks for a Semgrep security audit or fast pattern-based scan of a codebase, including broad or important-only coverage. Runs Semgrep, merges SARIF, and reports every finding with severity, file, and line; preserves all temporary clone paths for human disposition. Don''t use for remote, credential, publish, deploy, or irreversible changes.'
---
...
1. **Validate inputs.** Confirm `target` is a non-empty accessible path. If `coverage` is `important`, set `ruleset` filter to include only rules tagged with `security-high` or severity `ERROR` or `WARNING`; otherwise use `ruleset` as provided. Stop with failure class `invalid-input` if `target` is empty, missing, or unreadable.

2. **Plan scan.** Enumerate the set of scan units: one unit per ruleset being applied to each target scope. Record the expected unit count. This count is the accounting baseline.

3. **Clone rule sources (if applicable).** If `ruleset` references a remote registry path, clone it to a temporary local path under `/tmp/semgrep-rules-<uuid>/`. Record every cloned path in `CLONE_PATHS`. Do not delete these directories; leave them intact for human disposition.

4. **Run Semgrep.** Invoke `semgrep --json --no-gitignore --max-target-bytes 0 <ruleset> <target>` with any `--exclude` patterns provided. Capture stdout as `stdout_raw`. Capture stderr. Record exit code. If the command fails, record the unit as `failed` with the exit code and stderr excerpt.

5. **Collect unit results.** For each scan unit, record whether it produced output (`success`), produced no findings (`zero-findings`), produced malformed output (`malformed`), was skipped by Semgrep (`skipped`), or was not scoped (`unscoped`). Maintain a per-unit status map.

6. **Merge SARIF.** Parse every `stdout_raw` as JSON. If any unit produced malformed output, record the unit as `malformed` and proceed with valid units. For each valid unit, extract the SARIF `runs[].results` array and merge into a single `results` array, preserving all `properties`, `message`, `level`, `locations`, and `rulesMetadata` fields. Write the merged SARIF to `MERGED_SARIF`. If no valid units produced parseable JSON, set `MERGED_SARIF` to null.

7. **Build findings report.** If `MERGED_SARIF` is not null, parse it and extract one finding per result: severity, rule ID, message, file path, start line, and end line. Sort by severity (ERROR before WARNING) then by file path. If `MERGED_SARIF` is null, report zero findings.

8. **Accounting reconciliation.** Compare actual unit statuses against the expected unit count. Flag any unit that is unaccounted for as `unscoped`. Confirm the approved plan unit count matches actual unit count; if not, disclose the discrepancy.

9. **Report.** Emit a structured report containing: total findings count, findings list, per-unit status summary (failed/skipped/unscoped/zero-findings/malformed), the full `CLONE_PATHS` list, and the path to `MERGED_SARIF` if it was produced. The report is the skill output.

10. **Preserve clones.** Do not delete any path listed in `CLONE_PATHS`. The skill ends here; deletion is a human action.
...
```json
{
  "semgrep_version": "<semgrep --version output>",
  "target": "<target path>",
  "ruleset": "<ruleset used>",
  "coverage": "broad | important",
  "plan_units": <integer>,
  "actual_units": <integer>,
  "unit_statuses": {
    "<unit-key>": "success | failed | skipped | unscoped | zero-findings | malformed"
  },
