...
description: 'The one root-cause repair skill. Use when a bug, failure, flake, regression, or review finding needs the core fixed so it cannot break the same way again. Not for greenfield features, style-only review, or typo-class one-liners.'
---
...
|---|---|
| Trigger | A bug, failure, flake, regression, slowness, or review finding needs root-shape repair, not the nearest patch. |
...
- Optional: affected file or module to narrow the initial read set; earlier hypotheses or fixes already ruled out.
...
A confirmed root cause, the root-shape fix, a passing regression test, a clean tree with all instrumentation removed, the durability note, and each review-comment verdict — ordered locate → prove → repair → pin → verify → verdicts → durability. On non-convergence, a blocked report stating the irreproducible symptom or unisolated cause with the last captures, hypothesis, and artifacts tried.
