...
description: 'Use when the user asks to simplify or clean a specific code artifact, or when review flags bloat beyond the current diff. Reduces measured duplication or branch complexity while keeping the test suite green. Not for readability-only cleanup — use code-simplifier.'
---
...
1. Bound scope to the named target artifact. Do not edit files outside it or introduce new public surface. Done when: scope is bounded to one named artifact.
...
## Output
Simplified artifact plus report: baseline → final bloat signal → applied simplifications → reverted edits with reasons → final test result. Terminal classification: simplified, already-at-floor, or blocked.
