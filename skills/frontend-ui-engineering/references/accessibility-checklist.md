...
WCAG 2.1 AA compliance reference. Pairs with the `frontend-ui-engineering` skill. Markup examples are HTML — they apply to server-rendered templates and JavaScript frameworks alike (React uses `htmlFor`/`onClick` where plain HTML uses `for`/`onclick`).
