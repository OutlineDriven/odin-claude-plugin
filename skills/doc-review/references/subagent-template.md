...
<context-slots-rules>
- `Document type:` is the orchestrator's authoritative classification (`requirements` or `plan`). Trust it; do not re-classify by inspecting content shape. The orchestrator already used frontmatter, section structure, and content signals to decide.
- `Origin:` carries upstream provenance prepared by the orchestrator. It is the document's `origin:` frontmatter field when present, otherwise the literal token `none`. Read this line directly — do not parse the document's frontmatter yourself for this signal.
</context-slots-rules>
