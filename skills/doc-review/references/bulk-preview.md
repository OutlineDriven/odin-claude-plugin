...
1. **Routing option B (top-level best-judgment)** -- after the user picks `Auto-resolve with best judgment` from the routing question, but before any action executes. Scope: every pending `gated_auto` or `manual` finding at confidence anchor `75` or `100`.
...
Failure during `Proceed` follows the failure path defined in `references/open-questions-defer.md` -- surface the failure inline with Retry / Fall back / Convert to Skip, continue with the rest of the plan, and capture the failure in the completion report's failure section.
