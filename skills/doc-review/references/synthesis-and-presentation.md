...
|-------|-----------|--------|
| **3.1** Validate | Agent's returned JSON checked against the findings schema. | Drop findings missing a required field or with invalid enum values; note the offending agent in Coverage. Do not narrate schema/validation diagnostics to the user -- the only user-visible trace is a Coverage-row annotation (e.g. fewer findings, or a `malformed` marker). |
