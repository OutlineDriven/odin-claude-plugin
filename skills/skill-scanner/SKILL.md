---
name: skill-scanner
description: 'Use when a user asks to scan, audit, or validate a skill for security issues. Runs static analysis on skill files and returns a risk-level assessment with an install recommendation. Don''t use for tasks that require source or remote-system changes.'
---
...
1. Validate that `skill_path` exists and contains a SKILL.md file. If not, report `blocked` with the missing-path evidence and stop.
2. Read and parse the SKILL.md frontmatter. Extract name, description, and any invocation flags. If the file is malformed, note the parse failure and continue with remaining files.
3. Inventory every file under the skill directory. Record path, extension, and size.
...
5. For each finding, record: category, severity (low/medium/high/critical), file path, line range, matched pattern, and the surrounding context (two lines before and after).
...
7. Classify overall risk level as the maximum severity across all retained findings. If no findings remain after filtering, classify as `LOW`.
8. Compile the report (see Output section).
...
- **skill_name**: the parsed name from frontmatter.
- **file_count**: number of files inventoried.
- **findings**: array of objects, each with `category`, `severity`, `file`, `line_range`, `pattern`, and `context`.
- **false_positives_filtered**: count of findings removed during filtering.
- **recommendation**: `install` if risk is LOW or MEDIUM with no critical findings; `review-before-install` if HIGH; `do-not-install` if CRITICAL.
- **coverage_note**: `complete` if all files were analyzed, or a description of any files that could not be scanned.
