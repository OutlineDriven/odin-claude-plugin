...
|---|---|
| Trigger | User asks for customer feedback analysis, weekly summary, trending issues, NPS, churn, or email feedback over a window. |
...
| Side effect | Run the `analyze_feedback.py` analysis mechanism inline, save one markdown report under `reports/customer_feedback_summaries/`, then create a PR. The named mechanism is a behavior contract, not a dependency on an external script. |
| Done | The saved report contains themes, exact counts, an equal-length week-over-week comparison, cited GitHub issue links, and verbatim customer quotes, and the authorized PR contains that report. |
...
- A concrete account/team scope. For GitHub, include the allowed organization and repositories or team-owned repository set. For Gmail, NPS, and churn, include the account, workspace, tenant, team, cohort, or equivalent source-native restriction. Reject `all`, `*`, an unbounded mailbox, or any other wildcard scope.
- The enabled subset of GitHub issues, Gmail feedback, NPS responses, and churn records.
