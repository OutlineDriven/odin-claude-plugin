...
This file is loaded when the plan skill detects a non-software task (Phase 0.1b). It replaces the software-specific phases (0.2 through 5.1) with a domain-agnostic planning workflow.
...
> Wrong: "Quick note first: the plan skill builds implementation plans, so I ignored the template and just answered the question. Here's what the data says..."
...
2. Dispatch parallel research. Prefer user-named surfaces first; fall back to web search for questions those surfaces don't cover.
...
Ask up to 3 questions targeting the unknowns that would most change the plan. Use the platform's blocking question tool. Fall back to numbered options in chat only when no blocking tool exists or the call errors. Never silently skip the question.
...
## Step 3: Save or Present

After structuring the plan, ask the user how they want to receive it using the platform's blocking question tool. Fall back to numbered options in chat only when no blocking tool exists or the call errors. Never silently skip the question.
...
2. **Present in chat** -- Display the plan directly in the conversation. No file is written.

Do not offer implementation handoff (software-only) or issue creation (not applicable to non-software plans).
