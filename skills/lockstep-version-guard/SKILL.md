...
description: 'Use when a human invokes the release gate to prove all 29 ODIN packages share one canonical version. Emits a per-file comparison and exits non-zero on mismatch. Don''t use to edit release metadata or for remote, credential, publish, deploy, or irreversible changes.'
...
Supply the release root. The identity ledger `catalog/packages.json` under it must provide the canonical `releaseVersion` and enumerate the 29 ODIN package members, each identifying its version-bearing `package.json` and `.claude-plugin/plugin.json`. No input is optional.
...
2. Read the canonical version from the `releaseVersion` field of `catalog/packages.json`. Reject a missing, non-string, or empty version. Done when: the canonical version is read and confirmed as a non-empty string.

3. Read the `entries` array of `catalog/packages.json` and collect the version-bearing manifests each entry identifies (`paths.package_json` and `paths.plugin_manifest`). Require exactly 29 distinct ODIN package entries; reject duplicate, missing, out-of-root, or non-file paths. Do not discover or add unrelated files to make the check pass. Done when: the manifests for all 29 catalog entries are collected.
...
6. Exit 0 only if the catalog-coverage check passed (all 29 entries covered) and every record matches. Otherwise exit 1 and print every record, including matching records, so the release gate receives a complete per-file comparison. Done when: exit 0 (all match) or exit 1 (any mismatch) with the complete per-file listing printed.
