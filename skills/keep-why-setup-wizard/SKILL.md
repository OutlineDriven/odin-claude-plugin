...
| Authority | Reversible-local: write only named local entry-point files; rollback by restoring prior file content from version control or backup. |
...
2. **Handle legacy field backfill.** If the existing block is present but missing newer fields, backfill silently with documented defaults:
   ...
   Do not prompt the user for fields that have documented defaults.

3. **Run project wizard** (first activation only). Ask the user for project-level configuration. Write the pointer and project config block into the entry-point file.

4. **Run personal wizard** (personal block missing). Ask the user about confirmation flow and personal preferences. No silent default is applied because no prior behavior exists to preserve. Write the personal config block into the entry-point file.
...
6. **Evaluate timers opportunistically.** On each activation, compare elapsed time against configured intervals for update check and context staleness. No OS cron or background process is used.
...
8. **Validate written blocks.** Confirm both project and personal blocks are structurally valid in the entry-point file.
...
## Output
- Both project and personal config blocks present and valid in the entry-point file.
- Timer timestamps recorded for opportunistic comparison on next activation.
- Any update-check failure surfaced exactly once with a disable option offered.
- Report of blocks written, fields backfilled (if any), and timer evaluation results.
