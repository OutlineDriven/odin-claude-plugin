---
name: post-to-slack
description: 'Use when the user explicitly asks to share a message on Slack through an incoming webhook. Don''t use for posting without an explicit request or for retrying an ambiguous delivery.'
...
1. Confirm that the user explicitly requested this post and preserve the supplied message text without adding unsupported facts.
2. Parse the webhook URL before making any request. Require HTTPS, the exact host `hooks.slack.com`, and a non-empty path; reject credentials, query parameters, fragments, redirects, and every other host.
3. Preview the destination as the supplied label when available, otherwise as the configured Slack webhook destination, and show the exact message plus the consequence that one remote Slack message will be created. Do not display the webhook URL or any part of its secret path.
4. After the preview, send exactly one HTTP POST to the validated URL with `Content-Type: application/json` and the JSON object `{"text":"<message>"}`, where `<message>` is encoded as a JSON string.
5. Do not follow redirects or retry automatically. Treat only an HTTP 200 response whose body is exactly `ok` after trimming surrounding whitespace as confirmation.
6. Report the confirmed post. If confirmation is absent, stop with the applicable failure result and do not claim that the message was posted.
...
## Output
Return the destination label when supplied, a safe message summary or the exact message when appropriate, and one terminal classification: `posted` for confirmed HTTP 200 `ok`, `blocked` before mutation, `failed` for a confirmed rejection or pre-delivery transport failure, or `unknown` when delivery cannot be determined. Never include the webhook URL or secret path.
