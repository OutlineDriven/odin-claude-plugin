...
description: 'Use when a predicted Culture Index profile needs comparison with role requirements, team composition, and manager profile during an interview debrief, produce a confidence-weighted assessment with a recommendation and verification areas. Don''t use for tasks that require source or remote-system changes.'
---
...
1. Load the predicted profile. Record each trait (A, B, C, D as High/Low/Norm; L, I as 0–10), the predicted pattern, per-trait confidence, and the interview source and date. If any trait or its confidence is missing, mark it unknown and exclude it from weighted scoring.

2. Load role requirements. If a hiring profile exists, extract target traits, target pattern, and role red flags. Otherwise derive required trait directions from the role-fit answers: macro → A High, micro → A Low; people → B High, problems → B Low; high repetition → C High, low repetition → C Low; strict process → D High, flexible process → D Low. Record the target pattern and list traits that would struggle in the role.

3. Compare predicted versus required for each trait. Classify each as Y (strong match: same direction, similar magnitude), ~ (acceptable: within tolerance), or N (mismatch: opposite direction or extreme gap). Note any concern per trait.

4. Check predicted traits against role red flags. For each red flag, record whether the prediction hits it and severity (High/Medium/Low). Count total hits.

5. Assess team fit if team data is available. Count current Gas (High A), Brake (High D), and Glue (High B). Mark whether the candidate would add needed Gas, Brake, Glue, or perspective diversity. List friction risks where candidate traits oppose team member traits.

6. Assess manager fit if the manager profile is available. Compare manager versus predicted candidate positions for A, B, C, D and record the gap per trait. List predicted working-relationship alignment and friction points.

7. Weight confidence. For each dimension (role fit, team fit, red-flag hits, manager fit) rate the assessment Strong/Moderate/Weak and multiply by the dimension confidence (High → 3, Medium → 2, Low → 1) to get a weighted score. Apply caveats: low-confidence traits may change significantly when the actual survey is administered; medium-confidence traits are directionally correct but magnitude uncertain; high-confidence traits are likely accurate but interview stress may have affected them. Omit any dimension lacking its input rather than scoring it as zero.
