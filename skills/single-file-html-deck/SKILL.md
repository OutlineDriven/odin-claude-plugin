...
description: 'Use when an explicit request asks to create a presentation, build slides, make a deck, or build a Sentry presentation. Scaffolds a keyboard-navigable single self-contained HTML file with real-data charts and Sentry branding. Don''t use for remote, credential, publish, deploy, or irreversible changes.'
---
...
1. **Gather inputs.** Confirm topic, audience, and any source material. If topic or audience is missing, stop and request them before proceeding.
...
4. **Build slide content.** For each slide:
   ...
   - Add supporting body text kept concise (no more than three to four bullet points or one short paragraph).
...
9. **Resolve QA issues.** Fix each issue found in step 8. If a slide's content cannot be fixed without new information from the user, flag it as a blocker and deliver the deck with the blocker noted rather than shipping broken content.

10. **Deliver output.** Copy or move the built single HTML file to the requested location. Report the file path, slide count, and QA pass results.
...
- The full slide deck with keyboard navigation.
- Recharts visualizations using only real data (if provided).
- Sentry branding (colors, typography, logo).
