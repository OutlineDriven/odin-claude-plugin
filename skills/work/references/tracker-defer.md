...
Used by `/work` Residual Work Gate and similar caller flows when the user chooses to file tickets. All user-facing prompts fire:
...
- On total chain exhaustion, return each finding in the `failed` bucket for tiers that errored, and in the `no_sink` bucket only when no durable sink was ever available.
...
When Interactive mode's routing question is skipped entirely (zero-findings case), no probes run. When the cached tuple is reused across a session, any `named_sink_available = true` from the session's first probe stays cached — do not re-probe per Defer.
...
- **Length cap:** when the composed body would exceed a tracker's body length limit, truncate with `... (continued in /review run artifact: /tmp/odin/review/<run-id>/)` and include the finding_id in both the truncated body and the metadata block so the artifact is discoverable.
...
**Non-interactive mode:** do not prompt. Automatically fall through to the next tier. Record any tier that errored in the `failed` bucket with its reason. If every available tier errors, the finding ends up in `failed`. If no durable sink was ever available, the finding ends up in the `no_sink` bucket.

A named tracker gets one retry before it is considered broken. In Interactive mode, the user chooses whether to retry; in Non-interactive mode, retry once automatically. Only after the retry also fails is the cached `named_sink_available` set to `false` for the rest of the session. Subsequent Defer actions fall straight through to the next tier without retrying a confirmed-broken sink. `any_sink_available` is only downgraded to `false` when every tier has been confirmed broken — a failed Linear call that succeeds via `gh` keeps `any_sink_available = true`.
