...
- **HTML by default** — a single self-contained file (a verdict is a thing people share). Use markdown when the user asks, or when the write-up will feed `brainstorm` or further planning work.
...
- If the report is HTML, also render a throwaway markdown copy for any markdown-only consumer.
- Use an available HTML publishing tool the user has connected.
