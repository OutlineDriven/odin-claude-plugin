...
description: 'Use when setting up a project, auditing Claude Code settings, or asking which read-only bash commands to allow. Emits a recommended .claude/settings.json allow list and optional .mcp.json suggestions scoped to the detected stack. Don''t use for tasks that require source or remote-system changes.'
---
...
2. Detect services by reading dependency manifests: `package.json` (`dependencies`, `devDependencies`), `pyproject.toml` (`[project.dependencies]` or `[tool.poetry.dependencies]`), `Gemfile`, `Cargo.toml` (`[dependencies]`). Flag Sentry when `sentry-sdk`, `@sentry/*`, `.sentryclirc`, or `sentry.properties` is present; flag Linear when a `.linear/` directory or Linear config exists.

3. Read existing config: `cat .claude/settings.json` and `cat .mcp.json` (tolerate absence).

4. Build the allow list. Always include the baseline read-only commands, each as `Bash(<cmd>:*)`: `ls`, `pwd`, `find`, `file`, `stat`, `wc`, `head`, `tail`, `cat`, `tree`, `git status`, `git log`, `git diff`, `git show`, `git branch`, `git remote`, `git tag`, `git stash list`, `git rev-parse`, `gh pr view`, `gh pr list`, `gh pr checks`, `gh pr diff`, `gh issue view`, `gh issue list`, `gh run view`, `gh run list`, `gh run logs`, `gh repo view`, `gh api`.
...
- The done predicate is not met if any emitted command can modify state, contains an absolute path, or names a tool not detected in the repository; re-run step 8 and re-emit.
...
Adapted clean-room from `getsentry/skills` (`skills/claude-settings-audit/SKILL.md`, revision `c2f99a5b04b4cd992ec3022d7c2c3e23e938d241`, Apache-2.0). The Sentry-internal skill allow list and Sentry-only always-include domains were dropped; the read-only detection and local-config-emit mechanism is preserved.
