---
name: observability
description: 'Use when adding telemetry, reviewing alerting rules, shipping a production feature, or diagnosing an opaque production issue. Not for diagnosing a live failure right now, profiling measured slowness, or launch-day runbooks.'
---
...
|---|---|
| Trigger | Adding telemetry, reviewing alerting rules, shipping a production feature, or diagnosing an opaque production issue. |
...
2. Write down 2-4 on-call questions for the feature (for example: what fraction of attempts succeed on the first try; why does a permanent failure happen; is the provider slower than usual). Every signal added below must answer one of these questions. If no question can be named, stop and report; do not instrument. Done when: 2-4 on-call questions are written, each mapping to at least one signal below.
