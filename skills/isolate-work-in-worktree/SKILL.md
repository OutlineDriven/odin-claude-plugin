...
description: 'Use when a run needs its own branch and checkout so concurrent work cannot collide. Creates the loop/<runId> branch and an isolated worktree tracked in a manifest with a five-status lifecycle, and removes only terminal-status worktrees that git accepts without --force. Don''t use for untracked data or changes without a version-control rollback.'
---
...
6. Reconcile manifest and disk. List git worktrees with `git worktree list --porcelain` and compare paths under `.loop-worktrees/` against the manifest. Directories git knows but the manifest does not are orphans: report them, and remove one only on explicit instruction, still without `--force`. Manifest entries whose directory no longer exists are dropped from the manifest with an atomic write; treat a missing path as absent, not an error, since `rm -rf`, a crash mid-cleanup, or a container wipe can leave git listing a prunable entry.
...
- Git refuses `git worktree add -b` because `loop/<runId>` already exists: a leftover branch from a prior attempt with the same id; surface the refusal, then use a new id or delete the leftover branch explicitly.
