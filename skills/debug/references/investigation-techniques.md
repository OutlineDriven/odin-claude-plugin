...
- Remove setup steps that do not appear to affect the failing assertion
...
- Sampling profilers instead of tracing: external observation of what is running without injecting code into the path
- Platform-level instrumentation: `strace`, `dtrace`, eBPF, platform profilers that do not require code changes
...
**Preserve before investigating.** Error trackers and log systems have retention windows. Before starting a long investigation, export or snapshot the key evidence (event ID, trace ID, full stack trace, breadcrumbs) so it does not age out mid-session.
...
- **Floating-point precision:** comparisons that "should" be equal but are not, NaN propagating through a calculation and silently corrupting downstream results, very large or very small numbers losing precision
