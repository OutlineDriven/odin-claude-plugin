...
4. **If conflict with `~/.claude/claude/system-prompt-baseline.md`, system-prompt-baseline.md wins** — this skill complies with the user's git charter and tidy-first principles (compress operations under the Compressor/Extender role); if drift is detected, system-prompt-baseline.md is the source of truth.
