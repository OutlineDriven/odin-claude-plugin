---
name: agents-md
description: 'Use when asked to set up a repo for agents, add AGENTS.md, audit CLAUDE.md, make a repo agent-friendly, or score agent instructions. The repo ends with working, scored, non-contradictory AGENTS.md and CLAUDE.md for Claude, Codex, and Cursor. Don''t use for remote, credential, publish, deploy, or irreversible changes.'
---
...
|---|---|
| Trigger | User asks to set up a repo for agents, add AGENTS.md, audit CLAUDE.md, make a repo agent-friendly, or score agent instructions |
...
| Done | Repo has working, scored, non-contradictory AGENTS.md and CLAUDE.md for Claude, Codex, and Cursor |

## Inputs

- Required: the repository root path to audit or set up.
- Optional: target tool set (defaults to Claude Code, Codex, and Cursor); audit mode (quick or full, defaults to quick); target grade (defaults to 10/12 for quick, A for full).

## Procedure

AGENTS.md files are execution contracts, not knowledge bases. Every line must help an agent execute correctly with minimal context.

1. **Discover files.** Find every agent instruction file in the repo:
   ```bash
   find . \( -name "AGENTS.md" -o -name "CLAUDE.md" -o -name "CLAUDE.local.md" \) 2>/dev/null | sort
   ```
   Also check `~/.claude/CLAUDE.md` (applies to every session). For monorepos, include workspace-level files. Audit each level independently: root holds universal rules, child files hold directory-specific rules.
...
2. **Choose a mode.**
   ...
   - File exists and is bloated (root over ~150 lines), stale, or scored below target → **Refactor** (Step 5).
...
5. **Refactor — for bloated, stale, or low-signal files.**
   ...
   - Delete first, add back only what earns its place. For every removed line, record one reason: `generic`, `duplicate`, `stale`, `moved` (to a reference), or `reworded`. This log makes the final report traceable, and `reworded` keeps the safety re-check from treating a rewritten rule as lost content.
   - Remove: full documentation and tutorial-style prose, long architecture explanations in root, exhaustive file maps, generic advice ("write clean code", "use best practices"), outdated commands and dead links, restatements of default agent behavior ("read before editing", "run the tests after changes", "use the todo tool", "don't commit unless asked"), and facts auto-memory owns (user preferences, personal feedback, evolving project status).
   ...
   - Move non-universal detail to supporting files (`.agents/`, `docs/`, workspace-level AGENTS.md in monorepos), then link from root with `@import`. Guidance needed in fewer than ~30% of tasks moves out of root.
...
9. **Apply and report.** Apply approved edits, re-score with the same checklist, report before/after scores and line counts. Per future PR, add at most one new gotcha, only if it prevented or fixed a real mistake.
...
- Do not put project-specific commands in `~/.claude/CLAUDE.md`; it loads every session, so one project's the repository development command becomes noise or a wrong command everywhere else.
...
- **Partial result**: if the audit completes but validation fails, deliver the audit report with the failing validation checks marked. Do not apply any edits.
...
- For refactor: a deletion log with reason tags (`generic`, `duplicate`, `stale`, `moved`, `reworded`) and a before/after summary table.
...
- Adaptation: the source skill and its six reference files (project-setup, quick-checklist, quality-criteria, refactor-workflow, root-content-guidance, templates) were inlined into a single self-contained procedure. Cross-skill pointers and external tool references were removed. The 12-check quick checklist, 49-check full criteria with grade mapping and automatic fail conditions, import-trap portability rules, dead-weight and harmful-precision tests, refactor deletion log, skeleton templates, enforcement hierarchy, and validation smoke-run requirement were preserved as distinguishing mechanisms.
