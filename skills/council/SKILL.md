---
name: council
description: 'Use when the user asks for a council, second opinions, or parallel investigation, spawns parallel subagents that investigate the question from distinct stances and returns a decision memo with recommendation, consensus, risks, and evidence. Don''t use for tasks that require source or remote-system changes.'
---
...
1. Restate the question and confirm scope against the user-stated criteria. Bound the investigation before spawning; do not widen scope.
2. Define council members as distinct investigative stances appropriate to the question (for example optimist, skeptic, risk-auditor, domain expert). Use the user-specified stances when supplied; otherwise default to three.
3. Spawn one subagent per stance. Each subagent investigates the question independently from its assigned stance, gathers evidence, and returns a position with supporting evidence and identified risks.
4. Collect all subagent positions without merging prematurely.
5. Compare positions across stances: mark consensus points, disagreements, and unresolved risks.
6. Synthesize a decision memo stating the recommendation, the consensus reached, dissenting views, residual risks, and the evidence supporting each.
7. Present the memo. Do not select or execute a decision unless the user explicitly authorizes further action.
