---
name: readout
description: 'Use when a user wants a readable, shareable document of findings. Produces a self-contained HTML readout under ~/.readouts, embedding cited source text and refreshing the local index. Not for remote, credential, publish, deploy, or irreversible changes.'
---
...
2. Prepare rollback state. Ensure `~/.readouts/` exists. If `~/.readouts/index.html` exists, read and retain its exact bytes until the run succeeds. The only new primary path is `~/.readouts/<output_name>.html`. **Done when**: the directory exists and prior index bytes are retained if present.
3. Select the document shell. If `template_override` is absent, use this exact shell:
   ...
   **Done when**: a valid shell is selected with exactly four markers.
...
6. Write the readout exclusively. Create `~/.readouts/<output_name>.html` only if absent, write the complete UTF-8 document, and close it successfully. A partial file is deleted on any write or close failure. **Done when**: the file is written and closed successfully.
...
- **`directory-creation-failed`**: stop with the filesystem error.
...
- **`done-predicate-failed`**: restore the prior index and delete the new readout.
