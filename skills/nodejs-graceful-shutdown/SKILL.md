...
description: 'Use when asked to implement or fix service termination handling: SIGTERM/SIGINT, connection draining, health-check shutdown signaling, zero-downtime deploys. On signal the service stops accepting work, drains in-flight requests within the delay, closes external connections, exits cleanly; health endpoint observed flipping to 503 shutting_down; companion test passes. Don''t use for remote, credential, publish, deploy, or irreversible changes.'
---
...
1. Read every file in `SOURCES`. Identify the server bootstrap point: the `http.createServer` or Express/Fastify `listen` call. Identify any existing `process.on` handlers.
2. If no `isShuttingDown` boolean state exists, add it as `let isShuttingDown = false` at module scope.
3. Add or augment a `process.on('SIGTERM', shutdown)` and `process.on('SIGINT', shutdown)` handler. If the existing handler is named and registered, update it; do not register a second handler on the same signal.
...
6. Update every route handler that initiates async I/O (database calls, external HTTP) to check `if (isShuttingDown) return` and not start new work after shutdown has begun.
...
8. Run the test with `node --test` or the project's test runner. Assert all pass.
...
## Output
- Edited source files in `SOURCES` implement the graceful shutdown sequence described in Procedure.
- Companion test file at `<filename>.test.ts` passes, demonstrating the `503` health flip and clean `0` exit.
- No output artifact or report is emitted.
