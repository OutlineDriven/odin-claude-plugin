---
name: onboard
description: 'Use when the user asks for onboarding, orientation, a repository tour, or where to start. Produce an evidence-backed seven-section map, deep-read real files, and ask for the next move. Don''t use for tasks that require source or remote-system changes.'
---
...
1. Resolve the repository root, validate that it is an existing directory, normalize the depth, and record any focus. Stop with an input failure if the root or depth is invalid.
...
4. Attempt every applicable collection class. If evidence for a section is unavailable, state the reason once in that section rather than fabricating content or repeating the limitation.
...
6. End by asking the user to select exactly one next move, marking **Explore an area** as recommended for first-time onboarding:
   ...
   Use an available single-select interaction; otherwise present the four numbered options and stop after the question.
...
## Output
Return an in-chat orientation containing the applicable seven ordered sections, certainty labels where needed, a two-or-three-file source deep read with concrete paths and relationships, explicit degradations, and a single-select next-move question. The terminal classification is `complete` only when the root is valid, core evidence collection was attempted, at least one real source file was read, commands are grounded, and the next-move question is present; otherwise return `blocked` with the unmet predicate and the useful partial orientation.
