...
| Read.cv | `?` (unfillable) | `?` (unfillable) | `?` (unfillable) | `?` (unfillable) | `?` (unfillable) | `?` (unfillable) | Anti-LinkedIn neo-brutalism; acquired by Perplexity in 2025, fully shut down 2025-05-16 — design tokens never publicly published; only user-data exports were preserved before sunset, no token spec | https://web.archive.org/web/2025/https://read.cv |
| Anthropic | `oklch(0.982 0.005 95)` | `oklch(0.66 0.13 38)` | Tiempos Headline | DM Sans | DM Mono | `?` (no public spec) | "Stewardship" register; cream `#FAF9F5` + clay `#D97757` + near-black `#141413`; spacing-base not in any public design-system page — DevTools sampling shows ~16-24px rhythm consistent with 4px or 8px base | https://www.anthropic.com |
| Things 3 | `?` (NSColor.windowBackgroundColor — dynamic) | `oklch(0.865 0.177 90)` | SF Pro Display | SF Pro Text | SF Mono | 8px | Inherits macOS AppKit materials; `windowBackgroundColor` adapts per appearance (light ~`oklch(0.95 0 0)`, dark ~`oklch(0.18 0 0)`) and applies Desktop Tinting in dark mode; accent is `systemYellow` `#FFCC00` | https://culturedcode.com/things |
...
| lazygit | `?` (ANSI palette by design) | `?` (ANSI palette by design) | n/a (TUI) | n/a (TUI) | terminal mono | n/a | Dense panel grid for git ops; intentionally ANSI-pure (16-color names: green / red / cyan / blue) — author explicitly does not ship a hex palette so the user's terminal emulator owns the rendering | https://github.com/jesseduffield/lazygit |
| gh CLI | `?` (ANSI palette default) | `?` (ANSI palette default) | n/a (TUI) | n/a (TUI) | terminal mono | n/a | ANSI-degradation-aware status (green=success, red=error, yellow=warn) by default; `GH_COLOR_LABELS=1` opts into truecolor rendering of GitHub label hex; `GH_ACCESSIBLE_COLORS=1` (preview) uses customizable 4-bit accessible colors | https://cli.github.com/manual/gh_help_environment |
