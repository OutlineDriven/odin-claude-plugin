...
- **WidgetRef? `?`** — remains gated behind the `unstable-widget-ref` feature flag in 0.30; some sources describe it as stabilizing in the 0.30.x line, but the upstream API is still marked unstable. Treat as unstable until a 0.30.x point release lifts the flag.
...
- **App / Screen / Widget** — three-level hierarchy; widgets compose into screens, screens stack inside an app.
- **CSS-in-Python** — selectors, pseudo-classes, and a subset of CSS properties; not full CSS, but recognisable.
- **Accessibility `?`** — screen-reader status as of April 2026 is uncertain; do not claim accessibility compliance without testing against the user's reader of choice.
...
- **lazygit** — dense panel grid for git operations; multi-pane status with consistent left-aligned action labels and reserved hotkey columns. Trait: every region is interactive; dead pixels are rare.
- **helix** — modal editor; sparse default, accent-on-mode-change. The status line carries the action context (mode, register, count) instead of decorating the editing area. Trait: the chrome moves so the content can stay still.
- **gitui** — lazygit-shape layout with selective truecolor accents on diff regions (added/removed/context). Trait: color earns its keep where the eye must scan diffs; elsewhere it is restrained.

Anti-pattern across all three: a TUI that paints borders, separators, and inactive labels in saturated accent colors. Borders are subtle; accents fire where the user must look.
...
- **`NO_COLOR` env-var compliance** — when `NO_COLOR` is set (any value), all color is dropped and the surface remains usable on a 1-bit terminal. Layout must not depend on color to convey state.
...
This is starter density — defer to upstream changelogs for current capability surface before relying on a feature in production. Surfaces marked `?` need a primary-source check first.
