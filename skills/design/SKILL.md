---
name: design
description: 'Use when starting UI work, defining palettes or design tokens, or when output looks AI-generic, vibe-coded, or default-framework sloppy. Produces one defensible direction implemented consistently across palette, typography, spacing, density, and motion with runtime and cross-surface checks passing. Don''t use for remote, credential, publish, deploy, or irreversible changes.'
---
...
6. **Implement against the runtime.** Apply the cross-surface invariants regardless of runtime. Audit the result against the anti-slop charter.
...
## Output
One picked direction, a committed token set (color, type, space, radius, shadow, motion) expressed in the runtime's native token system, and implementation artifacts that reference those tokens. The result passes the anti-slop charter (neither Side A nor Side B) and the cross-surface invariants on the target runtime.
