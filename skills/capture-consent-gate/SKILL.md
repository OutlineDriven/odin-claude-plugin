...
description: 'Use when a user starts a recording before this process has acknowledged the what-gets-captured disclosure, withhold recording start until the disclosure is reviewed. Recording proceeds only after acknowledgement and every later start in the same process begins silently. Don''t use for remote, credential, publish, deploy, or irreversible changes.'
---
...
1. On receiving a recording-start request, check the in-memory reviewed flag for this process.
2. If the flag is already true, allow recording to start silently and stop.
3. If the flag is false, present the what-gets-captured disclosure to the user. The disclosure must state what data the recording captures, where it is stored, and who can access it.
4. Require explicit user acknowledgement of the disclosure before proceeding. Do not auto-acknowledge, default-accept, or allow recording to start without acknowledgement.
5. If the user acknowledges, set the in-memory reviewed flag to true and allow recording to start.
6. If the user declines or dismisses the disclosure without acknowledging, do not start recording. Leave the reviewed flag false.
