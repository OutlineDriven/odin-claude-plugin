...
Quick reference for application security. Use alongside the `security-and-hardening` skill. Code blocks appear in two language families; the controls are the same across stacks.
...
**Supply-chain hygiene** (a vulnerability audit will not catch malicious packages):
- [ ] Lockfile committed; CI uses a reproducible, locked install (`npm ci`, `pip install --require-hashes`, `cargo build --locked`)
...
- [ ] Model output treated as untrusted — never into `eval`/SQL/shell/raw-markup/file paths
