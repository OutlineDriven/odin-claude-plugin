...
description: 'Use when asked to codify, port, or audit a brand house prose style into a versioned PROSE.md. Output is an editor-applicable prose guide across five layers with per-rule rationale, plus an empirical AUDIT-MEMO.md when a corpus is available. Don''t use for remote, credential, publish, deploy, or irreversible changes.'
---
...
- **`TONE.md`** (optional) — emotional posture across four dimensions (funny↔serious, formal↔casual, respectful↔irreverent, enthusiastic↔matter-of-fact). Prose is the score; tone is the music. Two brands with identical tone can have non-interchangeable prose; this skill codifies the prose, not the tone. When `TONE.md` is missing, capture the four dimensions inline during the discovery interview.
- **Target channel grouping** (required for ADAPT) — long-form articles, social posts, email & newsletter, or marketing copy.

## Procedure

Prose is reproducible craft a forensic linguist could measure on a page: sentence length, clause depth, lexicon, parallelism, signature moves. Reason thoroughly on every BUILD and ADAPT invocation; shallow reasoning produces generic guides that flatten into LLM-default register, the exact failure this skill exists to prevent.
...
4. Run an adversarial reading pass on 3–5 representative pieces — challenge the assumption that they work. Mark every sentence that does not earn its place, every unanswered reader question, every moment authority collapses, every paragraph where a reader would disengage.
...
6. Write `AUDIT-MEMO.md`: 5–10 pages of quantitative tables, qualitative annotated samples, and a keep/kill/differentiate summary. Feed it into BUILD Phase 6.
...
4. **Structure** — opening hook types, closing types, transitions, headings (sentence case, frontloaded), subheadings, lists, asides, quotations, citations, blockquotes, reader positioning (Gardner's far↔close psychic distance: default per channel, shift-signal words, when to close for conversion).
...
1. Count lexical tells across the corpus — frequency ≥ 1 per 500 words is a strong tell.
2. Sentence-length σ < 4 across a 100-sentence window — uniformity is a stronger tell than any single lexical signal.
3. n-gram comparison between the brand's pre-AI and post-AI corpus — divergence in top trigrams flags drift.
...
4. Emit a `CHANNEL OVERRIDE — <grouping>` section appended to `PROSE.md`, or a standalone `PROSE-<grouping>.md` if the user prefers a separate artifact. Offer both because content teams publishing across many channels prefer one master file, while agencies handling a single channel prefer per-channel files.
...
## Output
- `PROSE.md` — a versioned, editor-applicable prose guide covering the five layers, punctuation and formatting policies, channel overrides, cultural/linguistic adaptation, and anti-LLM countermeasures, with a semver footer, owner, date, and changelog stub.
- `AUDIT-MEMO.md` — produced by AUDIT mode and by BUILD when a corpus is present; 5–10 pages of quantitative tables, qualitative annotated samples, and a keep/kill/differentiate summary.
- For ADAPT, either an appended `CHANNEL OVERRIDE — <grouping>` section in `PROSE.md` or a standalone `PROSE-<grouping>.md`.
