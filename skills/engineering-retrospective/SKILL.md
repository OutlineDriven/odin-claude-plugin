...
description: 'Use when the user runs /engineering-retrospective for a period. Produces an engineering retrospective with team breakdowns and habits assembled from telemetry and review logs. Not for an agent-environment retrospective — use agent-environment-retrospective; for a learning milestone — use learning-retrospective.'
---
...
|---|---|
| Trigger | The user runs /engineering-retrospective for a period. |
| Authority | Reversible local: write only the single engineering-retrospective report file; delete that file to roll back. |
| Side effect | One local write to the engineering-retrospective report file built from telemetry and review logs. |
| Done | An engineering retrospective with team breakdowns and habits is produced. |

## Inputs

- Period: the time window named in the /engineering-retrospective argument. Required; the engineering retrospective is bounded to it.
...
1. Read the period from the /engineering-retrospective argument and bound every later step to that window. Stop if no period is supplied.
...
6. Write the engineering-retrospective report to one local file containing the period, team breakdowns, habits, and the evidence each finding rests on.
...
## Output
One engineering-retrospective report file for the period, containing team breakdowns, habits, and the evidence behind each finding, plus the report file path and a one-line summary returned to the user.
