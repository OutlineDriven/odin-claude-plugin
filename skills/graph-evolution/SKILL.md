...
description: 'Use when the user provides two refs or source snapshots and needs security-relevant structural changes that a line diff may miss. Builds code graphs at both snapshots, runs pre-analysis, computes a structural diff, and returns a GRAPH_EVOLUTION report that classifies every node, edge, entrypoint, and subgraph change with stated limitations. Don''t use for remote, credential, publish, deploy, or irreversible changes.'
---
...
1. **Validate inputs.** Confirm both refs resolve or both directory paths exist. Confirm trailmark is installed by running `uv run trailmark --help`. If trailmark is not installed, run `uv tool install trailmark`; if installation fails, stop and report the error.

2. **Create snapshots.** If both inputs are git refs, create temporary worktrees from the repo root using `mktemp -d` directories: `git worktree add "$BEFORE_DIR" {before_ref}` and `git worktree add "$AFTER_DIR" {after_ref}`. If both inputs are directory paths, use them directly and skip worktree creation.

3. **Build graphs and run pre-analysis on both snapshots.** For each snapshot directory, build a Trailmark code graph, run pre-analysis, and export JSON:
   ```python
...
10. **Clean up worktrees.** After the report is written, remove temporary worktrees: `git worktree remove "$BEFORE_DIR"` and `git worktree remove "$AFTER_DIR"`. Account for their removal in the report methodology section.
