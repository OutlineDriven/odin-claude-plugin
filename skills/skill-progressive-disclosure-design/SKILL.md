...
description: 'Use when the model is creating or refactoring a skill, faces a SKILL.md over 300-400 lines, or confuses triggering with disclosure. Produce a justified split-or-monolith decision with pointer hygiene and an architecture-eval plan. Don''t use for remote, credential, publish, deploy, or irreversible changes.'
---

# Skill progressive disclosure design

Decide how to structure a skill's files, monolithic SKILL.md or split with reference files, for context efficiency and reliable triggering. Each recommendation includes explicit trade-offs; reason from them to the specific situation rather than applying rules blindly.
...
1. **Diagnose triggering vs. disclosure.** Two problems get conflated and must be separated before any splitting decision.
...
6. **Prefer scripts/ over references/ for deterministic work.** Anything deterministic (formatting, validation, schema generation, file transforms, regex-heavy parsing) belongs in `scripts/`: zero context cost for execution, deterministic output, reusable without re-reading. Scripts fail sharper: errors stop the workflow.
...
8. **Design the architecture evaluation.** Architecture evaluation is different from output evaluation. Output evals ask "did the skill produce the right thing?". Architecture evals ask "did the skill load the right files for the right reasons, at acceptable cost?".
...
## Output
A structured decision record containing:
1. **Diagnosis**: triggering vs. disclosure classification with evidence.
2. **Decision**: split or monolith, with explicit rationale citing the specific criteria from step 2.
3. **Architecture**: if split: the chosen axis, the file tree, and the exact pointer sentence for each reference file. If monolith: the rationale and any pruning recommendations.
4. **Evaluation plan**: the query set, instrumentation approach, metrics to collect, and decision thresholds from step 11.
