...
| Fan out independent work | `/skill:parallel-launch` → `/skill:do-it-now`, or `/skill:diamond-task` when one change is too large to split by worker | The tasks share mutable state or must run in a strict order. |
