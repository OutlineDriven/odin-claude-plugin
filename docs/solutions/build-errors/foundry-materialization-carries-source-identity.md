...
- A single `grep -r <old-name>` sweep. Bare prose words (beam, hate, aim) are not leaks, and attribution lines in `licenses/NOTICE` carry source names; a flat grep drowns in both. Each leak class needs its own pass and exemption list.
...
Build a name map (old source name -> new slug) from the rename ledger, then repair by class with dedicated detectors:
...
2. Catch invocation leaks: slash, backtick, and path forms of every old name, exempting attribution lines in `licenses/NOTICE` (a session-scope grep over the renamed slugs; no shipped gate exists for this class yet).
...
- `catalog/skill-membership.json` — the canonical neutral skill membership source mapping each slug to its package module.
- `licenses/NOTICE` — authored human attribution source; `packages/*/NOTICE` are generated copies.
